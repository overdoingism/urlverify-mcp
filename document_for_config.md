...
| `wikimedia_solo.enabled` | bool · `true` | Lets an old, widely read Wikimedia record establish an official domain on its own when it is the domain's only family. All of: the Wikidata official website and the English Wikipedia infobox (or its `{{Official URL}}`, which displays the Wikidata value) name the domain; a revision taken at a random time in `sample_window_months` (and itself made inside that window, and not the current revision) already named the same domain, on both; no change within `history_days`; the article had at least `min_monthly_views` human page views in each of the last 24 months. Any lookup that fails means the rule does not apply (two families needed as usual). A TRUE reached this way is capped at `max_confidence` with the notice `WIKIMEDIA_ONLY` (tell the user first). Costs up to three extra requests, only in that situation. |
