...
| `wikimedia_solo.enabled` | bool · `true` | Lets an old, widely read Wikimedia record establish an official domain on its own when it is the domain's only family. All of: the Wikidata official website and the English Wikipedia infobox (or its `{{Official URL}}`, which displays the Wikidata value) name the domain; the revision in effect at a random time in `sample_window_months` (not the current revision, i.e. the entry was edited since; the entry must already have existed then) already named the same domain, on both; no change within `history_days`; the article had at least `min_monthly_views` human page views in each of the last 24 months. Any lookup that fails means the rule does not apply (two families needed as usual). A TRUE reached this way is capped at `max_confidence` with the notice `WIKIMEDIA_ONLY` (tell the user first). Costs up to three extra requests, only in that situation. |
| `wikimedia_solo.min_monthly_views` | int · `2000` | Minimum human (non-bot) page views of the article in every one of the last 24 months. Raise it if automated browsing inflates counts. |
| `wikimedia_solo.sample_window_months` | [int, int] · `[18, 30]` | The random sample time is drawn from this many months ago. Random so a long-term plant cannot target a fixed date. |
| `wikimedia_solo.max_confidence` | float · `0.6` | Confidence cap of this path (the lowest of all TRUE paths). |
...
| `distro_archives` | list[str] · `[]` | apt: extra hosts (or `host/path` prefixes) that are mirrors of YOUR distribution, besides `*.debian.org` / `*.ubuntu.com`, e.g. `free.nchc.org.tw/ubuntu`. APT checks the distribution's signatures on any mirror; this only tells the tool that the mirror serves the distribution rather than a vendor repository. |
