...
- **老且多人閱讀的 Wikimedia 可單獨確立官網（`identity.wikimedia_solo`，2026-09-24 定案）**：網域只有 Wikimedia 一家時才檢查，全部成立才算：
  ...
  - Wikidata 與 Wikipedia **各自**在 18–30 個月前（`sample_window_months`）**隨機**抽一個時間點，取當時生效的版本：該版本本身必須在區間內
    （那段期間有人編輯；更早的就不採用），不能是目前版本（之後還有人編輯），且當時就是同一網域。近 `history_days` 內沒有變動。
