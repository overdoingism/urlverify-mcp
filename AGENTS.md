...
- **老且多人閱讀的 Wikimedia 可單獨確立官網（`identity.wikimedia_solo`，2026-09-24 定案）**：網域只有 Wikimedia 一家時才檢查，全部成立才算：
  - Wikidata P856 與英文 Wikipedia infobox 現在都指向該網域（比 eTLD+1，不比整串網址）。infobox 用 `{{Official URL}}`，或 `Infobox software`
    省略 website 欄位（模板會改顯示 Wikidata 值，已查模板原始碼），視為顯示 Wikidata 的值。
  - Wikidata 與 Wikipedia **各自**在 18–30 個月前（`sample_window_months`）**隨機**抽一個時間點，取當時生效的版本：不能是目前版本
    （之後還有人編輯），且當時就是同一網域；當時條目尚不存在則不採用。近 `history_days` 內沒有變動。
    該版本早於區間也照樣採用（2026-09-24 修訂）：版本越舊只代表值維持越久，潛伏成本更高；「沒人維護」已由「之後有人編輯」與瀏覽量門檻涵蓋，
    原本「版本須落在區間內」只會讓剛好那段沒人編輯的條目隨機失敗（7-Zip 實例），沒有換到安全性。
  - 英文條目過去 24 個月每個月的人類瀏覽量都 ≥ `min_monthly_views`（2000；歷史數據無法事後灌量；未來 AI 操作瀏覽器會墊高數字，可調高）。
  - 成立 → TRUE 信心上限 `max_confidence`（0.6，所有途徑最低），附 `WIKIMEDIA_ONLY`（列為需先告知使用者），寫入身分快取時一併保留上限與提示碼。
    任何一步查不到或不符 → 規則不適用，仍需兩家，不算失敗。只在第一次裁決不是 TRUE 時跑第二次（加入此規則），不讓它改變其他結果；
    有 LLM 時不因此跳過 LLM（讓它先找第二家）。抽樣結果存成 `wikimedia_solo` 紀錄（不引用、不列給 LLM、不投票），重播時沿用同一次抽樣。
  - **已知長尾風險**：有人在冷門時段潛伏超過兩年、期間密集編輯卻沒觸發頁面保護，就能讓假官網過關。隨機抽樣讓潛伏者無法對準固定日期，
    再加上瀏覽量門檻與最低信心，判斷為可接受。刻意不做「每台機器固定抽樣點」（太複雜）；代價是同一案可能這次過、下次不過（快取後就穩定）。
  - 範圍：這條規則回答「是不是官方」，不處理「官方網域是否已被挾持／過期搶註」（那屬另一個問題，見 §6.4 SourceForge 討論）。
