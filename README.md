...
| SourceForge | file URLs on `sourceforge.net/projects/<slug>` / `downloads.sourceforge.net`: treated as a mirror network. Established only when the project's independently established official site links the exact file, or links the SourceForge project whose record names that site as its homepage (TRUE capped at 0.8, `OFFICIAL_DOWNLOAD_HOST`); SourceForge's own automatic mirrors are `UNVERIFIABLE` (`SOURCEFORGE_MIRROR`) |
